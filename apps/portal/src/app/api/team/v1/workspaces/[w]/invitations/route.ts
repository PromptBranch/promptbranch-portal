...
import { listInvitations } from "@promptbranch/team-server";
...
/** GET /api/team/v1/workspaces/:w/invitations — owner only; no raw tokens. */
export async function GET(request: NextRequest, context: { params: Promise<{ w: string }> }) {
  ...
  try {
    ...
    return teamJson(requestId, { items: await listInvitations(service.pool, w), nextPageToken: null });
  } catch (error) {
